...
import numpy as np
import random
from .geo import convert_crs_shap, n_closest_geodetic
import pandas as pd
...
def mapbox_matrix_API(origins, destinations, token, epsg,
                      mode=1, name='poi', destination_id_col=False,
                      n_keep=3, do_all=False, verbose=True):
    """
    ...
    ori = origins.copy()
    o_type = 'hospital'
    cols = ['_'.join([prefix, o_type]) for prefix in ['t', 'm', 'so']]
    for col in cols:
        ori[col] = -1
    ...
    # only do empty ones
    if not do_all:
        queued_origins = ori[ori['t_hospital'] == -1]
    else:
        queued_origins = ori
    if verbose:
    ...
    for i in np.arange(queued_origins.shape[0] / batch):
        ...
        # to reduce API calls calculate keep only n closest (geodetically) to each origin.
        ...
        h_batch_loc = ";".join([",".join(
                      convert_crs_shap(row.centroid.x, row.centroid.y, epsg)) for row in h_batch['geometry']])

        # create url params of origin batch
        d = ";".join([",".join(
            convert_crs_shap(row.centroid.x, row.centroid.y, epsg)) for row in o_batch['geometry']])
    ...
    ori.loc[queued_origins.index, (cols)] = queued_origins.loc[:, (cols)].copy()
    print("returning")
